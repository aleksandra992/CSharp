/**
 * Created by Aleksandra on 6/8/2015.
 */
    ...
    function getNumbers(array)
    {
        ...
        for(var i=0;i<array.length;i++)
        {
            ...
            if(array[i] instanceof  Array)
            {
                ...
                for(var h=0;h<array[i].length;h++                )
                {
                  //  console.log('tuka');
                    numbers.push(array[i][h]);

                }
            }
            else
            numbers.push(variables[array[i]]);
        }
     //  console.log(numbers);
        return numbers;
    }

    function calculate(operator,arrayString)
    {
        var res=0;

       var array=arrayString.split(',').map(function(item){return item.trim()});
...
for(var i=0;i<commands.length;i++)
{
    ...
    var leftPart=currentRow.split('[')[0];
    var rightPart=currentRow.split('[')[1];
    rightPart=rightPart.trimRightChars(']');


    var leftParts=leftPart.split(' ').filter(function(x) {return !!x;})

    if(leftParts.length===3) {
        var functionName = leftParts[1];
        var operator=leftParts[2];

            variables[functionName]=calculate(operator,rightPart);
    ...
    {
        var variable=leftPart.split(' ')[1];


               var niza=rightPart.split(',').map(function(item){return item.trim();});
      // console.log(niza);
        variables[variable]=getNumbers(niza).map(Number);
       // console.log(variables);
    ...
    {
      //  console.log('tuka');
        result=variables[rightPart.trimRightChars(']')];

    }
    if(leftParts.length===1)
    {
        var operation=leftParts[0];
//console.log(operation);
        result=calculate(operation,rightPart);
